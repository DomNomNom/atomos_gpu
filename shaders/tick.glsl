...
ivec2 getTarget(ivec2 origin) {
    ...
    uvec4 target = texelFetch(connectionMap, originShifted, 0);
    // TODO: optimize this out by building a better map
    if (target.y == 0u) {
        return origin;
    }

    target.x += uint(maxCapacity);  // guarantee we don't get negative values
    target.x -= uint(getSliceShift(int(target.y), maxCapacity));
    target.x = target.x % uint(maxCapacity);

    return ivec2(target.xy);
}
